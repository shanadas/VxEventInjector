...
Collections::Generic::List<CPPCli::DataSource^>^ CPPCli::VXSystem::GetDataSources(System::String^ id) {
	...
	if (!String::IsNullOrWhiteSpace(id))
	{
		...
		dataSources.filterSize = 1;
		dataSources.filters = filters;
		dataSources.filters[0].key = VxSdk::VxCollectionFilterItem::kId;
		VxSdk::Utilities::StrCopySafe(dataSources.filters[0].value, Utils::ConvertSysString(id));
	}
...
Collections::Generic::List<CPPCli::Device^>^ CPPCli::VXSystem::GetDevices(String^ id) {
	...
	{
		VxSdk::VxCollectionFilter filters[1];
		devices.filterSize = 1;
		devices.filters = filters;
		devices.filters[0].key = VxSdk::VxCollectionFilterItem::kId;
		VxSdk::Utilities::StrCopySafe(devices.filters[0].value, Utils::ConvertSysString(id));
	}
...
List<CPPCli::Situation^>^ CPPCli::VXSystem::GetSituations(String^ type, String^ deviceId) {
	...
	if (!String::IsNullOrWhiteSpace(type))
		situations.filterSize++;
	if (!String::IsNullOrWhiteSpace(deviceId))
		situations.filterSize++;
	if (situations.filterSize > 0)
	{
		int idx = 0;
		situations.filters = new VxSdk::VxCollectionFilter[situations.filterSize];
		if (!String::IsNullOrWhiteSpace(type))
		{
			situations.filters[idx].key = VxSdk::VxCollectionFilterItem::kType;
			VxSdk::Utilities::StrCopySafe(situations.filters[idx++].value, Utils::ConvertSysString(type));
		}
		if (!String::IsNullOrWhiteSpace(deviceId))
		{
			situations.filters[idx].key = VxSdk::VxCollectionFilterItem::kSourceDeviceId;
			VxSdk::Utilities::StrCopySafe(situations.filters[idx++].value, Utils::ConvertSysString(deviceId));
		}
	}
